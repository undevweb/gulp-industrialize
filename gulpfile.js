...
if(argv.source != undefined){
    ...
    sPathSources = './config/'+ argv.source;
    ...
    if (oSources.js.active) {
        var iJs = 0;
        for (oRepos in oSources.js.repos) {
            sJsTask = 'js-minify' + String(iJs);
            jsTasks.push(sJsTask);
            self = oSources;
            self.oRepos = oRepos;
            gulp.task(sJsTask, ['clean'], function () {
                return gulp.src(self.js.repos[self.oRepos].src, {base: '.', cwd: self.cwd})
                            .pipe(jshint())
                            .pipe(gp_concat(self.js.repos[self.oRepos].dist))
                            .pipe(ngAnnotate())
                            .pipe(uglify())
    ...
    gulp.task('js-minify', jsTasks, function () {
        /*if (oSources.js.active) {
            for (oRepos in oSources.js.repos) {
                gulp.src(oSources.js.repos[oRepos].src, {base: '.', cwd: oSources.cwd})
                    .pipe(jshint())
                    .pipe(gp_concat(oSources.js.repos[oRepos].dist))
                    .pipe(ngAnnotate())
                    .pipe(uglify())
                    .pipe(rename({suffix: '.min'}))
                    .pipe(gulp.dest(PATH_DEPLOIEMENT));
    ...
    gulp.task('inject',['copy-index', 'copy-files','css-minify','css-sass', 'js-minify'],  function () {
